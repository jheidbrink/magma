...
void amf_ue_context_update_coll_keys(
  ...
  if ((gnb_ngap_id_key != INVALID_GNB_UE_NGAP_ID_KEY) &&
    ...
    m_rc = amf_ue_context_p->gnb_ue_ngap_id_ue_context_htbl.remove(ue_context_p->gnb_ngap_id_key);
    m_rc = amf_ue_context_p->gnb_ue_ngap_id_ue_context_htbl.insert(gnb_ngap_id_key,amf_ue_ngap_id);
    ...
    if (MAP_OK != m_rc) {
      OAILOG_ERROR_UE(
          ...
          ue_context_p->amf_ue_ngap_id,map_rc_code2string(m_rc)); //TODO: shashi
    }
    ue_context_p->gnb_ngap_id_key = gnb_ngap_id_key;
  }
  ...
  m_rc = amf_ue_context_p->imsi_amf_ue_id_htbl.remove(ue_context_p->amf_context.imsi64);
  ...
  m_rc= amf_ue_context_p->tun11_ue_context_htbl.remove(ue_context_p->amf_teid_n11);

  if (INVALID_AMF_UE_NGAP_ID != amf_ue_ngap_id) {
    m_rc = amf_ue_context_p->tun11_ue_context_htbl.insert(amf_teid_n11,amf_ue_ngap_id);
  } else {
    OAILOG_ERROR(
      LOG_AMF_APP,
      "Insertion of Hash entry failed for  "
  ...
  if (guti_p) {
    if ((guti_p->guamfi.amf_set_id !=
      ...
      if (INVALID_AMF_UE_NGAP_ID != amf_ue_ngap_id) {
        m_rc=amf_ue_context_p->guti_ue_context_htbl.insert(*guti_p,amf_ue_ngap_id);
      } else {
        OAILOG_ERROR(
          LOG_AMF_APP,
          "Insertion of Hash entry failed for  "
      ...
      if (ue_context_p) {
        ...
        if (ue_context_p->gnb_ngap_id_key != INVALID_GNB_UE_NGAP_ID_KEY) {
          ...
          ue_context_release_command(
           ...
           amf_app_desc_p->amf_ue_contexts.gnb_ue_ngap_id_ue_context_htbl.remove(ue_context_p->gnb_ngap_id_key);
