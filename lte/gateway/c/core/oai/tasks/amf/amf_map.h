...
namespace magma5g {
    ...
    */
    ...
    /*Enum for Map Return code
      Note: If new named constant is added to the enumeration list, add the new case in
      map_rc_code2string() located in amf_map.cpp*/
    ...
    **                                                                        **
    ***************************************************************************/
    template < typename keyT, typename valueT, class Hash = std::hash<keyT>,class KeyEqual = std::equal_to<keyT>>
    struct map_ts_s{
        ...
        void set_name(std::string umap_name){
            name=umap_name;
        }
        std::string get_name(){
            return name;
        ...
        ***************************************************************************/
        map_rc_t insert(const keyT key,const valueT value)
        {
            ...
            std::lock_guard<std::mutex> lock(mutex_obj);
            std::pair<itr,bool> insert_response =umap.insert( std::make_pair(key,value));
    ...
    char* map_rc_code2string(map_rc_t rc);
    ...
    //Note: This HashFunc in turn calls the def_hashfunc which supports hashing only for unit64
    // Char, string members cannot be hashed using this.
    struct HashFunc
    {
        std::size_t operator()(const guti_m5_t& k) const
        {
    ...
    struct equalToFunc{
        ...
        { 
            return (guti1.m_tmsi == guti1.m_tmsi);
        }
    };

    //Map Key: guti_m5_t Data: uint64_t;
    typedef map_ts_s<guti_m5_t,uint64_t,HashFunc,equalToFunc> obj_map_uint64_ts_t;
