...
int amf_context_upsert_imsi(amf_context_t* elm) {
  ...
  m_rc = amf_app_desc_p->amf_ue_contexts.imsi_amf_ue_id_htbl.remove(elm->imsi64);

  if (ue_id != INVALID_AMF_UE_NGAP_ID) {
    m_rc = amf_app_desc_p->amf_ue_contexts.imsi_amf_ue_id_htbl.insert(elm->imsi64, ue_id);
  } else {
    OAILOG_TRACE(
        LOG_AMF_APP,
        "Error could not update this ue context "
        "amf_ue_s1ap_id " AMF_UE_S1AP_ID_FMT " imsi " IMSI_64_FMT ": %s\n",
        ue_id, elm->imsi64, map_rc_code2string(m_rc));
    return RETURNerror;
  }
  return RETURNok;
}
...
void amf_free_ue_context(ue_m5gmm_context_s* ue_context_p) {
  ...
  amf_ue_context_p->imsi_amf_ue_id_htbl.remove(ue_context_p->amf_context.imsi64);

  amf_ue_context_p->tun11_ue_context_htbl.remove(ue_context_p->amf_teid_n11);

  amf_ue_context_p->guti_ue_context_htbl.remove(ue_context_p->amf_context.m5_guti);
